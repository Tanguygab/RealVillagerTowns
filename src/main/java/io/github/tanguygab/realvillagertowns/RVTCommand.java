        ...
        switch (arg) {
            case "skin" -> {
                if (Utils.getTarget(p) instanceof LivingEntity v && rvt.entEnabled(v)) {
                    String skin = rvt.saveFile.getString("villagers." + v.getUniqueId() + ".skin");
                    sendMessage(sender, "&eThat villagers skin is: " + skin);
                } else sendMessage(sender, "&cYour not looking at a villager!");
            }
            case "setvillager" -> {
                if (rvt.getConfig().getBoolean("autoChangeLivingEntitys")) {
                    sendMessage(sender, "&cLivingEntitys auto change!");
                    return;
                }
                if (Utils.getTarget(p) instanceof LivingEntity v && rvt.entEnabled(v)) {
        ...
        switch (arg) {
            case "fixskin" -> {
                if (Utils.getTarget(p) instanceof LivingEntity v && rvt.entEnabled(v)) {
                    ...
                    String oldSkin = rvt.saveFile.getString("villagers." + v.getUniqueId() + ".skin");
            ...
            case "setprofession" -> {
                if (!(Utils.getTarget(p) instanceof Villager v)) {
                    sendMessage(sender,"&cYour not looking at a villager!");
                    return;
                }
                try {
