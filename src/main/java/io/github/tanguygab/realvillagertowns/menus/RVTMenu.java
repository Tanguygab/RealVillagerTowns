...
public abstract class RVTMenu {
    ...
    public final static Map<Player,RVTMenu> openedMenus = new HashMap<>();

    protected final Player player;
    protected final RealVillagerTowns rvt = RealVillagerTowns.getInstance();
    public Inventory inv;

    public RVTMenu(Player player, String title, int rows) {
        this.player = player;
        inv = Bukkit.getServer().createInventory(null,rows*9,Utils.colors(title));
        openedMenus.put(player,this);
    }
    public RVTMenu(Player player, String title, InventoryType invType) {
        this.player = player;
